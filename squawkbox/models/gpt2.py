...
class GPT2_Standard(nn.Module):
    ...
    def __init__(
        ...
        vocab_size,
        n_positions,
        n_ctx, # integer multiple of chunk_size, how many previous hidden states to keep track of
        chunk_size, # the number of steps to be processed in a single pass
        n_embd,
        n_layers,
        n_head,
        layer_norm_epsilon,
    ...
    def forward(self, src, tgt = None, timestamps = None, hidden = None, **kwargs):
        hidden_out, presents = self.main_model(
            input_ids = src, 
